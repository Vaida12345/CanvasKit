...
public struct SelectByColor: SelectionCriteria {
    ...
    /// Sample count passed to Metal as `ushort`.
    let sampleSize: UInt16
    
    
    /// Selects pixels matching the target color with weighted multisampling.
...
public extension SelectionCriteria where Self == SelectByColor {
    ...
    static func color(_ color: PartialColor, tolerance: Float = 0.01, sampleSize: Int = 3) -> SelectByColor {
        let clamped = UInt16(max(1, min(sampleSize, Int(UInt16.max))))
        return SelectByColor(color: color, tolerance: tolerance, sampleSize: clamped)
    }
